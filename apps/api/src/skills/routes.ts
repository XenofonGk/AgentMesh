...
const VersionParam = z.object({ name: SkillNameSchema, version: z.coerce.number().int().positive() });
...
const EvaluateVersionBody = z.object({
  testSet: z.array(z.object({ task: z.string().min(1), providers: z.array(z.string().min(1)).optional() })).min(1),
...
export async function registerSkillRoutes(
   ...
   */
  server.post('/skills/:name/versions/:version/evaluate', guard, async (request, reply) => {
    ...
    const candidateRow = await getSkillVersion(db, params.data.name, params.data.version);
    if (!candidateRow) {
      return reply.code(404).send({ error: 'not_found' });
    }
    const candidateParsed = parseSkillMarkdown(candidateRow.content, 'candidate');
    if (!candidateParsed.ok) {
      return reply.code(400).send({ error: 'invalid_skill', details: candidateParsed.error });
    }
    ...
    if (activeRow) {
      ...
      if (!activeParsed.ok) {
        return reply.code(400).send({ error: 'invalid_active_skill', details: activeParsed.error });
      }
      ...
      if (!parsed.ok) {
        return reply.code(400).send({ error: 'invalid_active_skill', details: parsed.error });
      }
    ...
    await recordSkillVersionEvalResult(db, params.data.name, params.data.version, report);

    return reply.send({ report });
  });

  /** GATE: promotes a `'proposed'` version to `'active'` and writes it to the live file. */
  server.post('/skills/:name/versions/:version/activate', guard, async (request, reply) => {
    const params = VersionParam.safeParse(request.params);
    if (!params.success) {
      return reply.code(400).send({ error: 'invalid_params' });
    }

    const activated = await activateSkillVersion(db, params.data.name, params.data.version);
    if (!activated) {
      return reply.code(404).send({ error: 'not_found_or_not_proposed' });
    }

    await mkdir(resolve(skillsDir, params.data.name), { recursive: true });
    await writeFile(skillMdPath(skillsDir, params.data.name), activated.content, 'utf8');
