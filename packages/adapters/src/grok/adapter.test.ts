/**
 * `fetch` is mocked with a real `ReadableStream` body — same reasoning as
 * `deepseek/adapter.test.ts`: this adapter really streams, so the SSE-chunk-splitting
 * logic (`sseData`) is worth exercising against real stream chunk boundaries.
 */
...
describe('grokAdapter', () => {
  ...
  it('sends the run token as Authorization: Bearer through the proxy, requests streaming', async () => {
    ...
    const events = [];
    for await (const event of grokAdapter.run({ task: 'say hi' }, ctx())) {
      events.push(event);
    }

    expect(fetchMock).toHaveBeenCalledTimes(1);
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('http://proxy:3002/providers/grok/v1/chat/completions');
